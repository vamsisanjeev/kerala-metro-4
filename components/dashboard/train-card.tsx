...
import { Brain as TrainIcon, ChevronDown, ChevronUp, Gauge, Clock, Paintbrush, Activity, Sparkles, ClipboardCheck } from 'lucide-react';
        ...
        <div className="p-5">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-lg ${config.color} shadow-md`}>
                <TrainIcon className="h-5 w-5 text-white" />
              </div>
              <div>
                <h3 className="text-xl font-bold text-white">{train.trainId}</h3>
                <Badge className={`${config.color} text-white border-0 mt-1`}>
                  {train.status}
                </Badge>
              </div>
            </div>
